...
export default function ZapPlayer({...props}) {
  const startArray = new Array(props.program.length).fill(false);
  ...
  useEffect(() => {
    if(readyVideoList.length === props.program.length && 
  ...
  return (
    <div className={allVideosReady ? 'zapPlayer' : 'loading'}>
      <>{
        props.program.map((video, index) => {
            return <TikTokPlayer key={index} id={video.id} start={video.start} end={video.end} play={playVideoList[index]} onReady={() => onReady(index)} onEnded={() => setPlayVideo(index + 1)}/>
          }
        )
      }</>
      { !allVideosReady ? 
        (<>
          <div>
          <LinearProgress variant="determinate" value={100 * (readyVideoList || []).filter(x => x).length / readyVideoList.length  } />
          </div>
          <div className="parentCenteredChild absolutely">
              Buffering video's<br/><br/>
        </div>
       </>)
      : (!playVideoList.reduce((a,b) => a || b,false) && 
         <div className="parentCenteredChild">
           
              <PlayArrowIcon className="playButton" onClick={() => setPlayVideo(0)} />
           
         </div>)
      }
      { finished && (<div className="parentCenteredChild">
        <p>Thank you for watching!<br/>
          Refresh for new video's.<br/><br/>
